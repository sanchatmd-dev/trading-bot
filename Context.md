...
Current phase status (2026-09-26): **APP-3A Bridge engineering accepted in staging; next work is QL-2A**. Exact Spot v4 compile/capture evidence, 24 PostgreSQL checks, 22 focused Node checks and a hosted controlled Paper BUY/targeted EXIT against real collector bars are recorded in [the acceptance record](docs/APP_3A_ACCEPTANCE_2026-09-26.md). Readiness/activation were exercised only on a separate test Bot/database, then stopped and cleaned up. The original SPT deployment remains DRAFT/capture-only, production is unchanged and SPT Quant remains UNSUPPORTED. QL-2A must build/prove the evaluator and reproduce Paper costs, sizing, exits and rejection before optimization. SMTP remediation and unknown provider accounting remain separate follow-ups.
