...
test('Bridge HTTP endpoints enforce auth/CSRF/owner scope and strict early strategy rejection',async t=>{
  ...
  const inspectRoute='/api/quant/pine-bridge/inspect',inspectBody={bot_id:a.id,pine_source:source};
  assert.equal((await request(inspectRoute,'POST',inspectBody)).status,401);
  assert.equal((await request(inspectRoute,'POST',{...inspectBody,bot_id:b},session)).status,404);
  const inspected=await request(inspectRoute,'POST',inspectBody,session);
  assert.equal(inspected.status,200,JSON.stringify(inspected.body));
  assert.equal(inspected.body.source_hash,hash(source));
  assert.equal(inspected.body.input_count,1);
  assert.equal(inspected.body.inputs[0].default_hint,10);
  ...
  const reviewed=await request(route,'POST',{...body,effective_inputs:{[inspected.body.inputs[0].input_id]:12},input_review:{source_hash:inspected.body.source_hash,confirmed:true}},session,{'Idempotency-Key':randomUUID()});
  assert.equal(reviewed.status,202,JSON.stringify(reviewed.body));
  const stored=await db.prepare('SELECT analysis FROM pine_source_revisions WHERE pine_import_id=? AND source_version=1').get(reviewed.body.pine_import_id);
  assert.equal(stored.analysis.effective_input_review.reviewed_by,a.id);
  assert.equal(stored.analysis.inputs[0].effective_value,12);
  ...
  await request('/api/quant/pine-bridge/jobs/'+reviewed.body.job_id+'/cancel','POST',{},session);
});
test('owner-reviewed Paper re-arm preserves policy and daily guard after UTC rollover',async()=>{
  const a=await owner(),policy=await store.risk(a,config.defaultRisk),account='binance-global:primary';
  await store.transitionBotState(a,'run',policy,[]);
  await db.prepare("INSERT INTO ledger_streak(user_id,account_id,execution_mode,loss_streak) VALUES(?,?,'PAPER',?)").run(a,account,policy.pauseAfterLossStreak);
  await assert.rejects(store.rearmLossStreak(a,a,'binance-global','reviewed recent Paper losses'),/Stop the Bot/);
  await store.transitionBotState(a,'stop',policy,[]);
  const today=new Date().toISOString().slice(0,10);
  await db.prepare("INSERT INTO ledger_daily(user_id,account_id,execution_mode,day,trades,notional,realized_r) VALUES(?,?,'PAPER',?,1,100,-1)").run(a,account,today);
  await assert.rejects(store.rearmLossStreak(a,a,'binance-global','reviewed recent Paper losses'),/next UTC day/);
  assert.equal((await store.lossStreaks(a))[0].has_today_activity,true);
  await db.prepare("DELETE FROM ledger_daily WHERE user_id=? AND account_id=? AND execution_mode='PAPER' AND day=?").run(a,account,today);
  const result=await store.rearmLossStreak(a,a,'binance-global','reviewed recent Paper losses');
  assert.equal(result.previous_loss_streak,policy.pauseAfterLossStreak);
  assert.equal((await store.lossStreaks(a))[0].loss_streak,0);
  assert.equal((await store.getBotSession(a)).state,'STOPPED');
  assert.deepEqual(await store.risk(a,config.defaultRisk),policy);
  const audit=await db.prepare("SELECT * FROM audit WHERE user_id=? AND event='bot.risk.loss_streak_rearmed' ORDER BY id DESC LIMIT 1").get(a);
  assert.equal(JSON.parse(audit.details).reason,'reviewed recent Paper losses');
  await assert.rejects(store.rearmLossStreak(a,a,'binance-global','reviewed recent Paper losses'),/Loss-streak pause is not active/);
});
