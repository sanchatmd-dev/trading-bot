...
test('restart quarantines LIVE processing orders instead of resending',t=>{
  const {store,user,filename}=fixture(t);
  const row=claim(store,user,signal(),'LIVE');store.persistIntent(row,{quantity:10});store.close();
