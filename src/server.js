...
import {Auth,SecurityMailWorker} from './auth.js';
...
const auth=new Auth(store,config,json),securityMail=new SecurityMailWorker(store,config,notifier);
const requireSession=req=>auth.require(req);
const requireAdmin=(req,res,url)=>{const user=auth.require(req),permission=adminPermission(req.method,url.pathname);if(!permission||!hasPermission(user,permission)){json(res,403,{error:'Permission denied'});return null;}auth.sensitive(req);return user;};
...
async function userRoutes(req,res,url){const actor=requireSession(req,res);if(!actor)return;
  if(!hasPermission(actor,req.method==='GET'?'own:read':'own:write'))return json(res,403,{error:'Permission denied'});
  if((req.method!=='GET'&&(url.pathname==='/api/me/webhook-secret'||url.pathname.startsWith('/api/brokers/')))||url.pathname==='/api/me/password')auth.sensitive(req);
  ...
  if(req.method==='POST'&&url.pathname==='/api/bots'){
    ...
    const bot=store.createBot(actor.id,body.label,{...config.defaultRisk,paperTrading:true},id=>store.setWebhookSecret(id,secret,encryptJson({secret},config.keyring,`webhook:${id}`)));
  ...
  if(url.pathname.startsWith('/api/analytics/')){
    ...
    if(!hasPermission(actor,'analytics:any')&&!store.ownsBot(actor.id,requested))return json(res,403,{error:'Cannot access another user analytics'});
    if(!store.ownsBot(actor.id,requested))auth.sensitive(req);
  ...
  if(req.method==='GET'&&url.pathname==='/api/me/webhook-secret'){
    ...
    const {secret}=decryptJson(saved.webhook_secret_encrypted,config.keyring,`webhook:${user.id}`);
  ...
  if(req.method==='PUT'&&url.pathname==='/api/me/webhook-secret'){
    ...
    store.rememberWebhookSecret(user.id,secret,encryptJson({secret},config.keyring,`webhook:${user.id}`));
    store.audit(user.id,'webhook.secret.recovered',null,{});
    return json(res,200,{urlPath:`/webhooks/tradingview/${secret}`});
  }
  if(req.method==='POST'&&url.pathname==='/api/me/webhook-secret'){const secret=randomToken();store.setWebhookSecret(user.id,secret,encryptJson({secret},config.keyring,`webhook:${user.id}`));store.audit(actor.id,'webhook.secret.rotated',null,{botId:user.id});return json(res,200,{secret,urlPath:`/webhooks/tradingview/${secret}`});}
  ...
  if(req.method==='POST'&&url.pathname==='/api/me/password'){
    if(!auth.limit('password-change:'+actor.id,10))return json(res,429,{error:'Too many verification attempts'});
    const body=await readJson(req),full=store.userByEmail(user.email);
    if(!await verifyPassword(body.currentPassword,full.password_hash))return json(res,400,{error:'Current password is incorrect'});
    const passwordHash=await hashPassword(body.newPassword);req.validateAuth();
    store.setPassword(user.id,passwordHash);auth.writeCookie(res,auth.cookieName,'',0);
    store.audit(actor.id,'account.password.changed',null,{});return json(res,200,{ok:true});
  }
  ...
  if(req.method==='PUT'&&url.pathname.startsWith('/api/brokers/')){
    ...
    store.setCredential(user.id,broker,encryptJson(credentials,config.keyring,`${user.id}:${broker}`),enabled);
...
async function adminRoutes(req,res,url){const admin=requireAdmin(req,res,url);if(!admin)return;
  ...
  if(req.method==='POST'&&url.pathname==='/api/admin/users'){const body=await readJson(req),email=String(body.email||'').trim().toLowerCase();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)||email.length>254)return json(res,400,{error:'Valid email required'});const role=body.role||'USER';if(!['USER','SUPPORT','ADMIN'].includes(role))return json(res,400,{error:'Invalid role'});const passwordHash=await hashPassword(body.password);req.validateAuth();const user=store.createUser({email,passwordHash,role});store.setRisk(user.id,{...config.defaultRisk,paperTrading:true});store.audit(admin.id,'admin.user.created',null,{userId:user.id,email,role});return json(res,201,user);}
  if(req.method==='PUT'&&/^\/api\/admin\/users\/[^/]+\/role$/.test(url.pathname)){
    const id=url.pathname.split('/')[4],body=await readJson(req),target=store.userById(id);
    if(!target||target.parent_user_id)return json(res,404,{error:'User not found'});
    if(id===admin.id)return json(res,400,{error:'Cannot change your own role'});
    if(!['USER','SUPPORT','ADMIN'].includes(body.role))return json(res,400,{error:'Invalid role'});
    store.db.exec('BEGIN IMMEDIATE');
    try{store.db.prepare('UPDATE users SET role=? WHERE id=?').run(body.role,id);store.revokeSessions(id);store.audit(admin.id,'admin.user.role_changed',null,{userId:id,role:body.role});store.db.exec('COMMIT');}catch(error){store.db.exec('ROLLBACK');throw error;}
    return json(res,200,{id,role:body.role});
  }
  if(req.method==='PUT'&&/^\/api\/admin\/users\/[^/]+\/status$/.test(url.pathname)){const id=url.pathname.split('/')[4],body=await readJson(req),status=String(body.status).toUpperCase(),target=store.userById(id);if(!target||target.parent_user_id)return json(res,404,{error:'User not found'});if(!['ACTIVE','SUSPENDED'].includes(status))return json(res,400,{error:'Invalid status'});if(id===admin.id&&status!=='ACTIVE')return json(res,400,{error:'Cannot suspend current admin'});store.setUserStatus(id,status);store.audit(admin.id,'admin.user.status_changed',null,{userId:id,status});return json(res,200,{id,status});}
...
const server=http.createServer(async(req,res)=>{res.setHeader('x-content-type-options','nosniff');res.setHeader('x-frame-options','DENY');res.setHeader('referrer-policy','no-referrer');res.setHeader('content-security-policy',"default-src 'self'; style-src 'self'; script-src 'self'; connect-src 'self'; base-uri 'none'; object-src 'none'; frame-ancestors 'none'; form-action 'self'");if(config.secureCookies)res.setHeader('strict-transport-security','max-age=31536000');const url=new URL(req.url,'http://localhost');try{
  ...
  if(req.method==='POST'&&url.pathname.startsWith('/webhooks/tradingview/')){
    ...
    let owner=store.botOwner(user.id);
    ...
    const current=store.userByWebhook(secret);owner=store.botOwner(user.id);
    if(current?.id!==user.id||current.status!=='ACTIVE'||owner?.status!=='ACTIVE')return json(res,404,{error:'Not found'});
...
}catch(error){const authPath=url.pathname.startsWith('/api/auth/');store.audit(null,'request.error',null,{message:authPath?'Authentication request failed':error.message,path:url.pathname.startsWith('/webhooks/')?'/webhooks/[redacted]':authPath?'/api/auth/[redacted]':'/api/[redacted]'});if(!res.headersSent)json(res,error.status||400,{error:error.message,...(error.code?{code:error.code}:{})});}});
server.requestTimeout=15000;server.headersTimeout=10000;server.maxRequestsPerSocket=100;
worker.start();notifications.start();securityMail.start();
...
const shutdown=async()=>{
  ...
  await Promise.all([worker.stop(),notifications.stop(),securityMail.stop(),closed]);
