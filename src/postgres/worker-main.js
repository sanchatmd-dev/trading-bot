...
import {PineMarketWaitWorker} from './pine-market-wait-worker.js';
...
if(process.env.PINE_BRIDGE_ENABLED==='1') {
  ...
  marketWait=new PineMarketWaitWorker({store,defaultRisk:config.defaultRisk});
  pineBridge.start();
  marketWait.start();
}
...
let stopping=false;
async function stop(){if(stopping)return;stopping=true;await Promise.all([worker.stop(),mail.stop(),pineBridge?.stop(),marketWait?.stop()]);await db.close();}
