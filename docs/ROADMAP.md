...
Implementation update (2026-09-24): **APP-3A is in progress**. Draft APIs/UI, durable direct-AI jobs, source revisions, evidence-controlled activation, trusted market ingestion and the scoped Paper receiver/worker are implemented locally. Isolated PostgreSQL acceptance and a simple TradingView fixture passed. Actual provider delivery, SPT compilation/native-alert isolation and TradingView-to-market/receiver evidence remain open. Feature defaults off; APP-3A is not complete and QL-2A is not unlocked. See [APP-3A implementation record](APP_3A_IMPLEMENTATION.md).
