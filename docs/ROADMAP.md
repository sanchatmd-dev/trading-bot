...
| APP-3 | Universal Risk Manager runtime, multi-indicator isolation and measured Paper scaling | Bot Lifecycle & Session Management implemented and verified (PostgreSQL Schema 14: bot_sessions, bot_session_archive, policy freeze, worker gating, 12 integration tests passed, all CI workflows green); Paper scaling and physical acceptance ongoing. |
...
| QL-4 | Reconciled reports, Pine export and Paper forward validation | Implemented and hardened in repo (offline SVG HTML tear sheets, Pine v6 exporter, input preset updater, webhook metadata pass-through, audit hardening with 4 P1 blocker fixes; 70/70 pytest, 111/111 Node tests passed); TradingView/VPS Paper forward acceptance pending. |
