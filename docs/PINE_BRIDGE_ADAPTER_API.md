...
### Spot-only trading and Futures indicator adaptation

Trading Bot execution and Quant Lab evaluation support **Spot trading only**: funded long BUY and reduce-only SELL/EXIT. No short inventory, borrowing, leverage, margin, liquidation, funding payments, perpetual/futures contracts or futures order routes may enter Bot execution, optimization or export. Both stages must use the same approved Spot instrument, market bars, position rules and cost assumptions.

An uploaded `indicator()` may still describe Futures trading. The AI chatbot must identify futures-specific symbols/data, short-entry or hedge/reverse branches, leverage/margin sizing, funding/liquidation logic and contract quantity assumptions. It must produce a **Spot-adapted indicator proposal**: retain valid long-entry logic; turn an owner-approved opposite/SELL signal into a reduce-only exit of an existing long; remove futures-only orders, sizing and economics; and recompute any dependent signal or risk logic using Spot data and units. Never reinterpret a futures-specific value as a Spot value without an explicit rule. If a Spot equivalent cannot be established, return a blocker and request a source decision instead of guessing.

Show the proposed Pine source and a line-level change/behavior summary to the user for review. Store the submitted source/hash and the approved Spot-adapted source/hash as distinct immutable revisions. **Append the Bridge only to the approved Spot revision**, preserving that revision byte-for-byte before the appended block. Recheck compile, effective inputs, signal mapping, parity and Quant support against the approved Spot revision; no equivalence to the submitted Futures indicator is implied. `strategy()` remains rejected for user conversion before this indicator-only adaptation flow.

This adaptation flow is a requirement, **not yet an implemented APP-3A capability**. The current AI API returns mapping JSON and appends a Bridge to unchanged source; it cannot produce or register revised Pine. Until an approved adaptation workflow exists, Futures-dependent indicators must remain blocked from Spot Webhook ready, Paper execution, Quant optimization and export. SPT's owner-approved `sellSignal`-as-long-exit mapping is recorded below; that mapping alone does not prove all other source dependencies are Spot compatible.
...
| Spot venue identity | Require the exact approved TradingView venue/instrument identity, not ticker text alone. `bridge-exit-v2` pins `binance-global` to `BINANCE:<symbol>`; other brokers require reviewed chart-ID mappings before Spot execution readiness. Existing v1 alerts must be replaced with a new deployment before inheriting this guard. |
...
Use the supplied versioned Bridge template, AI-facing guide, indicator-bridge-v2 and bridge-exit-v2 for new drafts. Existing v1 artifacts keep their frozen receiver behavior. Propose signal mappings, numeric candidates and a collision-safe block for Backend assembly; never rewrite the original body. Keep Bridge and Quant capabilities separate: MTF/pivots or a missing evaluator do not alone block append-only generation. Preserve those calculations and report Quant blockers independently.

Trading Bot and Quant Lab are Spot-only. For a submitted Futures-oriented indicator, propose a separate Spot-adapted indicator and explain every changed signal, symbol/data dependency, sizing rule and removed Futures assumption. Require user review and register the approved revision before appending Bridge. Do not silently patch the submitted source inside a Bridge job or claim Futures-to-Spot equivalence. Until the separate adaptation capability is implemented, return `SPOT_ADAPTATION_REQUIRED` and block Spot execution/optimization for Futures-dependent inputs.

The Bridge is long-only. A selected SELL or opposite signal produces a reduce-only EXIT for existing Bridge long entries; it must never create a short entry or short position. For the owner-approved SPT source, map `buySignal` to long BUY and `sellSignal` to long EXIT only, even though SPT defines `sellSignal` as a short-entry signal. Preserve SPT's `Both Long & Short` direction input so that `sellSignal` is still generated. Do not apply these exact variable names to other indicators without reviewing their semantics. Keep the source's simulated backtest trades and SL/TP separate from Bridge execution rules.
...
The runtime AI instructions use `bridge-prompt-v3` and `bridge-guide-v3` for the Spot-only requirement and long-only SPT mapping rule. The current JSON-only runtime can diagnose Futures dependencies but cannot perform the proposed source adaptation; that capability remains separate work. Previously generated drafts retain their recorded instruction versions and require a fresh reviewed generation if the mapping or source inputs change.
...
| Route | Body / behavior |
| --- | --- |
| `POST /api/quant/pine-bridge/inspect` | Authenticated, owner-scoped local Pine inspection; accepts `bot_id`, `pine_source` and returns source hash, input identities/types/domains and literal default hints. No AI call or source registration occurs. Incomplete scans fail closed. |
| `POST /api/quant/pine-bridge/analyze` | Initial source: `bot_id`, `pine_source`, `source_name`; reviewed Chatbot path also sends a complete typed `effective_inputs` map and `input_review:{source_hash,confirmed:true}`. Revision: additionally `pine_import_id` and next integer `source_version`. Requires `Idempotency-Key`; returns durable job identity. Legacy drafts without review remain possible but do not qualify the Custom evaluator. |
...
| `POST /webhooks/pine-bridge/v2/{secret}` | New artifacts use `bridge-exit-v2`. A missing verified closed bar enters `WAITING_MARKET` for at most 5 seconds, capped by the Bot's existing signal-age limit. HTTP 202 acknowledges storage only; no Paper signal exists yet. A separate worker rechecks deployment, owner/Bot status, policy, funding, evidence, market hash, close/ATR, SL/TP and signal age before queuing. Invalid existing market data or wait expiry rejects the intent. Duplicate event IDs retain the first outcome. Existing v1 routes retain immediate missing-bar rejection. |
...
The Chatbot's reviewed path calls `inspect` first, displays every input, and requires explicit confirmation against the TradingView indicator settings before `analyze`. It sends an exact `input_id → typed effective value` map and `input_review:{source_hash,confirmed:true}`. Backend rejects an incomplete map or changed source hash, records reviewer/time and a canonical effective-input hash with the source revision, and freezes those values for binding/export. Changing source, Bot or any displayed input clears the UI confirmation. The exact approved SPT Spot v4 source has a convenience fill for the `Custom` reference profile, but the owner still sets and confirms the same values in TradingView. The inspect step never sends source to AI; after confirmation, analyze sends the authorized source as before. Provider metadata omits nonnumeric owner values except the bounded SPT preset, so notification text and IDs from the reviewed settings are not duplicated in the AI request. Source code itself is sent during analyze and must be authorized and free of credentials. Quant Custom rejects snapshots without the matching review record. This local implementation needs rollout and fresh Custom parity evidence before runtime support.
...
Operational timing gate for v2: a matching independently collected closed bar must be frozen **before Paper queue admission**, not necessarily before HTTP receipt. Record receipt, wait deadline, verified-bar check/queue time, rejection outcome and latency in the v2 intake row for both immediate and delayed admission. Do not count late-arriving bars as ready at intake. The bounded wait does not relax signal-age, identity, Spot-only, price/ATR, risk, evidence or repaint gates. Binance Spot closed-kline WebSocket (`x=true`) is the low-latency feed; the REST collector fills gaps after reconnect and reconciles frozen OHLCV without rewriting old bars. Launch the WebSocket as a dedicated market-writer service and retain a slower REST fallback. This collection mode remains staging work until its continuity and latency are measured.
