...
```mermaid
flowchart TD
    S1["1. User เลือก Indicator / Strategy<br/>(Pine Script ใดๆ)"] --> S2["2. สร้าง Pine Bridge Adapter<br/>(ผูกสัญญาณและ Input เดิม)"]
    ...
    S4 --> S5["5. Quant Lab Export ค่า Input ที่ดีที่สุด<br/>(Best Inputs + Email Report)"]
    S5 --> S1
```
...
| **2. ติดตั้ง Robot Bridge ท้ายสคริปต์** | Signal Layer & Pine Bridge Adapter API | รับ Pine source ที่ผู้ใช้มีสิทธิ์ใช้งาน วิเคราะห์สัญญาณและ Input เดิม สร้าง Pine Script ที่ผูก Bridge แบบ 1:1 พร้อมคู่มือตั้งค่า webhook; `indicator` เพิ่ม Bridge ต่อท้าย ส่วน `strategy` แปลงเป็น `indicator` เฉพาะกรณีที่พิสูจน์ความเทียบเท่าของตรรกะและสัญญาณได้ |
...
| **5. Quant Lab Export ค่า Input ที่ดีที่สุด** | QL-4 (Pine Export) | ส่งมอบ 2 รายการ: (1) Best Inputs package: `inputs.json`, Pine Script และ Setup Guide สำหรับ TradingView; (2) Email Report สรุป Best Inputs เพื่อให้ผู้ใช้ตรวจและนำไปอัปเดตหน้า Bot Risk Manager โดยระบุ `bot_id`, `pine_import_id`, Timestamp และ Metrics สำคัญ |

### Step 2 — Pine Script Bridge Adapter API (new scope)

Build a lightweight chatbot-backed API that prepares the initial integrated Pine Script and concise Robot Trade webhook setup guide **before** Bot execution and Quant research. This Step 2 artifact is distinct from Step 5's export of optimized inputs and validation evidence. Detailed request/response design and the chatbot system prompt are in [PINE_BRIDGE_ADAPTER_API.md](PINE_BRIDGE_ADAPTER_API.md).

- For an accessible `indicator`, keep the original source and signal logic intact and append a namespaced transport Bridge at the end. Require explicit binding to the source's existing BUY/exit conditions; never infer an executable order from a visual marker alone.
- For an accessible `strategy`, produce an `indicator` copy only when its signal, state, timing and exit behavior can be translated with demonstrated equivalence. Keep the original source immutable, disclose the declaration/strategy API changes, and return `UNSUPPORTED_CONVERSION` when exact behavior cannot be preserved. Native strategy order-fill alerts do not exist in the converted indicator.
- Always append independent Bridge `ATR Multiplier for SL` and `RR` inputs with versioned defaults `2.0` and `1.5`. Never reuse or replace user Pine ATR/SL/RR variables. Source inputs retain separate 1:1 bindings. One Pine / one Bot optimizes every strategy input parameter plus the Bridge pair and exports the complete optimized set; multiple Pine scripts / one Bot freeze source inputs and search/export only one shared best Bridge ATR/RR pair. Count connected scripts, not internal indicators. The earlier 10-parameter ceiling is superseded. Define domains and bounds for every strategy parameter; unsupported parameters block a complete optimized export. Hard Bot risk limits remain outside optimization.
- Return complete integrated Pine, an input-binding manifest, source hash and a short setup guide. The server resolves Bot ownership and risk policy; generated code and chatbot output cannot change execution limits or activate a Bot.
- Acceptance: review the source diff and 1:1 bindings, compile the generated code in TradingView, compare original versus adapted signals on supported bars, then send a controlled Paper webhook. Record unsupported source constructs and missing parity evidence as explicit blockers.

## Sequence and dependencies

Default delivery order: **R-0 → R-1 → QL-1 → QL-2 → APP-3 → QL-3 → QL-4 → BR-1 → APP-4 → APP-5**. BR-1 is a newly proposed implementation milestone for workflow Step 2; its workflow position does not imply it was delivered with the earlier QL-4 work.
...
| BR-1 | Step 2 Pine Script Bridge Adapter API and chatbot prompt | Planned: generate an integrated source and setup guide from authorized Pine source; enforce 1:1 source bindings and independent Bridge ATR/RR inputs; optimize all strategy parameters for one Pine and only the Bridge pair for multiple Pine scripts; verify indicator append integrity and gate strategy conversion on signal/state parity and TradingView compilation. |
...
- Keep broker/risk policy fixed during an optimization run. The independent Bridge ATR/RR pair is always a supported search requirement. With one Pine, every strategy input parameter must participate in optimization; with multiple Pine scripts, all source inputs remain fixed. A separate experiment is required for a different execution/risk policy.
...
- Keep Optimization Bounds separate from Risk Policy. UI, evaluator and export must use the same server-resolved Pine-per-Bot mode: one Pine optimizes every strategy input parameter plus the Bridge pair and exports all optimized values; multiple Pine scripts optimize/export only the independent Bridge ATR/RR pair. Both dimensions must affect the evaluator before enabling this workflow.
...
- Deliver exactly two user-facing Step 5 outputs: **Best Inputs** (`inputs.json`, supported Pine Script and TradingView Setup Guide) and an **Email Report** containing the recommended input values and key Quant Lab evidence for the selected Bot Risk Manager.
- For one Pine / one Bot, Best Inputs exports all parameters after optimizing every strategy input parameter and the independent Bridge ATR/RR pair. Do not silently retain unsearched defaults or claim complete optimization when any strategy parameter is unsupported. For multiple Pine scripts / one Bot, actionable JSON, Pine edits and Email Report recommendations contain only the best shared Bridge ATR/RR pair. A Pine artifact may retain unchanged user code but must not retune source inputs. Keep all source/version/input snapshots privately as research provenance, with participating Pine identities listed in report metadata.
- Create a server-issued `pine_import_id` for each immutable Best Inputs package and bind it to `bot_id`, `run_id`, source/strategy version and effective risk-policy snapshot. Include the ID and UTC generation timestamp in the report and package metadata; make report creation/outbox enqueue idempotent and make duplicate at-least-once email deliveries recognizable by report ID.
- Send the report only to the verified owner of the selected Bot. Include key train/validation/test metrics, objective, trade/sample count, drawdown, Win Rate, Profit Factor, cost assumptions, validation status and stale status where applicable. Separate Risk Manager-compatible fields (such as supported SL/ATR and RR values) from indicator inputs (such as EMA/RSI) that belong in Pine. Link to the authenticated Bot Risk Manager import/review view; never email webhook secrets or broker credentials.
- Treat emailed values as a reviewable recommendation. Email delivery must not update Bot Risk Policy or activate a Bot. Applying values requires an explicit authenticated import/review action, server-side policy and bound validation, and an audit record; hard risk limits and guards remain unchanged.
...
Done when: identical fixtures reproduce Node FIFO Win Rate/Profit Factor definitions, reports reconcile capital and PnL, return metrics use the declared equity basis, and reports contain provenance and no credentials or cross-user data. Step 5 delivers both the Best Inputs package and an idempotent owner-scoped Email Report with `bot_id`, `pine_import_id`, UTC timestamp, best input values, key metrics and validation/stale status. The report separates Bot Risk Manager fields from Pine-only indicator inputs. Applying compatible values to Bot Risk Manager requires an explicit authenticated review and audit record; email itself never changes policy. Both user-selectable export modes compile and pass their own recorded Paper forward checks: an eligible event executes at most once, duplicate retries do not create another order, exits cannot oversell, rejected entries are visible, and stale/unknown deployment handling does not orphan existing-position exits. Unsupported or unverified combinations are not marked Validated. Reports remain private until authenticated report delivery is implemented and verified.
