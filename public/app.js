...
async function api(path,o={}){
  ...
  const r=await fetch(path,{...request,headers:{'content-type':'application/json',...(token?{authorization:`Bearer ${token}`} :{}),...(request.headers||{})}}),d=await r.json();if(!r.ok)throw new Error(d.error||'Request failed');if(!silent&&request.method&&request.method!=='GET'&&!path.startsWith('/api/auth/'))showSaved();return d;
}
