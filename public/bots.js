...
function renderBots(){
  ...
  $('#botSwitcher').innerHTML=botProfiles.map(bot=>`<option value="${esc(bot.id)}">${bot.bot_slot_index}. ${esc(bot.label)}</option>`).join('')+`<option value="all">${botText('All Bots','รวมทุก Bot')}</option>`;
