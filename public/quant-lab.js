...
function qCandidateRow(candidate, index) { const p = candidate.params || {}, status = candidate.status || (candidate.passed_stress && candidate.stability_ok ? 'passed' : 'rejected'); return `<tr><td>${index + 1}</td><td>EMA ${qText(p.ema_fast)}/${qText(p.ema_slow)}</td><td>${qFixed(candidate.train_score)}</td><td>${qFixed(candidate.validation_score)}</td><td>${qFixed(candidate.test_score)}</td><td><span class="ql-candidate-status ${status === 'passed' ? 'passed' : ''}">${status}</span></td></tr>`; }
function qShowOptimizer(result) {
  ...
  const intro = best ? `<section class="ql-best-params"><div><span class="ql-mini-label">Best validated candidate</span><h3>EMA ${best.params.ema_fast} / ${best.params.ema_slow}</h3><p>ATR ${best.params.atr_period} × ${best.params.atr_multiplier} · train ${qFixed(best.train_score)} · validation ${qFixed(best.validation_score)} · test ${qFixed(best.test_score)}</p></div><span class="ql-candidate-status passed">passed</span></section>` : '<p class="ql-empty-state">No candidate passed every validation gate. Adjust bounds or budget and run again.</p>';
...
async function qOptimize() {
  if (!qOptimizeValid()) return; const button = q('qlOptimize'); qSetBusy(button, true, 'Running sweep…'); qStatus('Running constrained optimization…');
  try { const result = await api('/api/quant/optimize', {method: 'POST', body: JSON.stringify({bounds: {ema_fast_min: qNum('qlFastMin'), ema_fast_max: qNum('qlFastMax'), ema_slow_min: qNum('qlSlowMin'), ema_slow_max: qNum('qlSlowMax')}, config: {num_candles: qNum('qlOptCandles'), max_candidates: qNum('qlCandidates')}})}); qShowOptimizer(result); q('qlOptMeta').textContent = `Complete · ${qFixed(result.total_evaluated, 0)} candidates`; qStatus(`Optimization complete: ${result.total_evaluated} candidates.`, 'ok'); } catch (error) { qStatus(error.message || 'Optimizer unavailable.', 'err'); } finally { qSetBusy(button, false); }
}
...
function initQuantLab() {
  ...
  ['qlEmaFast', 'qlEmaSlow', 'qlAtrPeriod', 'qlAtrMult', 'qlCandles', 'qlBalance'].forEach(id => q(id).addEventListener('input', qBacktestValid)); ['qlFastMin', 'qlFastMax', 'qlSlowMin', 'qlSlowMax', 'qlOptCandles', 'qlCandidates'].forEach(id => q(id).addEventListener('input', qOptimizeValid));
  document.querySelector('[data-view="quant"]').addEventListener('click', async () => { try { const health = await api('/api/quant/health'); q('qlEngineStatus').textContent = `Ready · ${health.mode}`; q('qlEngineStatus').className = 'ql-engine-state ready'; qStatus(`Quant engine ready · ${health.mode}`, 'ok'); } catch { q('qlEngineStatus').textContent = 'Offline'; q('qlEngineStatus').className = 'ql-engine-state offline'; qStatus('Quant engine offline. Research functions are unavailable.', 'err'); } });
}
initQuantLab();
